...
- `site/static/` — Static assets (images in `site/static/img/`, charts in `site/static/img/charts/`, PDFs, manifest)
...
- `.github/workflows/ci.yml` — CI pipeline (path-filtered Go lint/test, JS test, integration tests)
- `compose.yaml` — Local dev stack (Traefik + blog + cc-live)
- `compose.test.yaml` — Integration test stack (cc-live only)
- `scripts/plots/` — timberline-plots Python charting library
- `tests/integration/` — Go integration tests for cc-live
- `.golangci.yml` — Go linter configuration
- `package.json` / `vitest.config.js` — JS test configuration (vitest + jsdom)
...
make push-cc-live     # Build + push cc-live
make deploy-cc-live   # Build + push cc-live + pulumi up

make build-all        # Build blog + cc-live
make push-all         # Push blog + cc-live
make deploy-all       # Deploy blog + cc-live together

make login            # Podman login to registry
